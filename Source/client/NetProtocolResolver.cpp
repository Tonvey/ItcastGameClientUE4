...
GameMsgArray_t NetProtocolResolver::ResolveMessage()
{
    ...
    while (mLastBuf.size() >= 8)
    {
        ...
        if (mLastBuf.size() - 8 >= len)
        {
            ...
            //产生一个SingletTLV
            FGameSingleTLV msg((GameMsgID_t)msgType, msgContent);
...
void NetProtocolResolver::AppendMessageToString(const FGameSingleTLV &singlePbMsg,std::string &dest)
{
...
void NetProtocolResolver::PushMsg(const FGameSingleTLV& msg)
{
    std::string sendBuf;
